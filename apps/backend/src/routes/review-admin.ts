...
// this list entirely.
// plan.md Item 23 (2026-09-15): phone now lives on business_owners, not
// businesses -- INNER JOIN is safe (businesses.owner_id is NOT NULL).
reviewAdminRouter.get("/businesses", async (c) => {
  ...
  const rows = await queryAll<{ id: string; name: string; phone: string; name_fa: string | null; manual_editor_enabled: number }>(
    db,
    `SELECT b.id, b.name, bo.phone, bc.name_fa, b.manual_editor_enabled
     FROM businesses b
