...
20. **Natural-language campaign editing (chat-driven) + wizard "thinking" polish -- decided 2026-09-14.** Two related but separately-scoped pieces, both grounded in infra that already exists (`ai-models.config.ts`'s free-tier Gemini-Flash-first cascade, and the `suggested_changes` table + `SuggestionsTab.tsx` Apply/Dismiss flow). **Parts A and C are built and merged (see below); Part B is not yet started.**
    ...
    **Suggested build order:** ~~Part C first (no dependencies, purely cosmetic, ships same day).~~ Done. ~~Then Part A (backend engine + the stateful-history decision -- KV, per the decision above).~~ Done. Then Part B (wires Part A into `CampaignEditorTab.tsx` + `SuggestionsTab.tsx`) -- next up.
