...
**Admin storage + bootstrap — decided 2026-09-11:** a **separate `review_admin` role with its own login route**, not a boolean flag on `review_team_members` — keeps the regular `review_team` OTP path (and the `review_team_members` invite-only check) completely untouched by admin logic. The main admin's phone is **env-configured** (e.g. an `ADMIN_PHONE` secret checked directly by the admin route), not a seeded `review_team_members` row — this is what actually solves the bootstrap problem: the very first admin can't be invited by anyone (there's no one to invite them), so checking against env config sidesteps the roster table entirely for that one identity, rather than needing a special hardcoded first-row hack inside the same table meant to be invite-only for everyone else. Any *further* admins beyond the one main admin aren't designed yet (out of scope for now — v1 assumes a single main admin). Still not yet designed: the admin route's actual registration endpoint/UI for adding `review_team_members` rows (a review-console equivalent of business-owner's `POST /api/business/staff`), and the `review_admin` role's own JWT/session shape.
...
6. **`review_team` OTP verification has no roster/invite check** — unlike `staff`, any phone number can authenticate as `role: 'review_team'` (see Phase 0.5 "Security finding"). **Decided 2026-09-11: fix via `review_team_members`, built invite-only** (mirroring `staff`'s invite gate exactly), not just an audit-only lookup table; not yet implemented — still needs the migration and the `verify-otp` route change to check/reject against it. **Admin decided 2026-09-11: a minimal admin role on top of `review_team`** — some review-team members can register/deactivate others (self-service within the team), rather than a hardcoded/seeded phone list (rejected: every new hire would need a manual migration) or leaving it to direct DB access only (rejected: doesn't scale past a single operator and isn't really "invite-only" if there's no in-product way to invite). Not yet designed: how the admin flag is stored (`review_team_members.is_admin`? a separate role value?), whether the first admin is seeded via migration to bootstrap the system, and what UI/endpoint an admin uses to register a new phone (a review-console equivalent of `POST /api/business/staff`).
