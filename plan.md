...
**Step 5 done, branch `customer-app-scaffold`:** Customer persona app — join/OTP (dev OTP `5432`, optional referral-code capture at signup), code/QR + points + referral progress + carryover display, task list per verification method, reward redemption with a 5-minute countdown, retroactive purchase claim, notifications feed. No `AppShell` — the customer flow is single-screen/linear, unlike Business Owner's multi-tab accordion dashboard, so `App.tsx` routes straight to `CustomerHome`. `submitRetroactivePurchaseClaim`/`processReferralSignup` ported from `mockup/customer.html` + `mockup/shared/app.js` as pure functions over React state (`src/lib/mock-data.ts`), same reworking pattern as Step 4 (no global mutation).

**Step 6 done, branch `staff-pos-scaffold`:** Staff POS PWA persona app. Auth decided via user choice (asked since plan.md didn't specify it): **shared-device PIN** (dev PIN `2468`, distinct from the phone+OTP pattern used elsewhere) — not phone+OTP, not no-auth, since this is a shared till device. `vite-plugin-pwa` added (manifest + `generateSW`) since this persona needs installable-PWA scope per the deployment architecture rationale. 3 tabs (purchase/fulfill/queue), 1:1 port of `mockup/staff-pos.html`'s flows including Gap #9's offline-queue dedup-by-idempotency-key + validity check (`syncOfflineQueue` pure function in `src/lib/mock-data.ts`).

**Step 7 done, branch `review-console-scaffold`:** Review Console persona app (central team). Auth decided via user choice (asked since plan.md didn't specify it and the mockup hardcodes `reviewed_by` as a fixed `'central_team'` string) — **phone+OTP per team member** (dev OTP `9911`), giving the audit trail a real per-person identity instead of one shared label; see "Review Console authentication" under Phase 0.5 above. Two sections, 1:1 port of `mockup/review-console.html` + `mockup/shared/app.js`'s `resolveSubmission`/`runReferralAnomalyDetection`/`resolveFlag`, reworked as pure functions over React state in `src/lib/mock-data.ts`: (1) uncertain-AI-submissions + retroactive-purchase-claims queue with Approve/Reject, (2) referral anomaly flags queue with a "Run Batch Job" trigger and Dismiss/Mark-Reviewed actions. The real anomaly-detection batch job needs cross-customer aggregate data this app doesn't have standalone, so `runReferralAnomalyDetection` operates over a stub `referrerAggregates` array shaped like what the real backend endpoint would return (seeded to reproduce the mockup's two test cases: one referrer over the velocity threshold, one over the dead-referral-ratio threshold) — same simplification approach Step 5 used for `processReferralSignup`.

**BRANCH CONVERGENCE NOTE:** Steps 5, 6, and 7 were each built on their own branch, forked fresh off `main` as siblings (not stacked on each other) — `customer-app-scaffold`, `staff-pos-scaffold`, `review-console-scaffold`. Each branch's `.github/workflows/deploy.yml` only has the jobs that existed on `main` at the time it forked (`business-owner`) plus its own new job (`customer`, `staff_pos`, or `review_console` respectively) — none of the three branches has all three new jobs. Merging any two of these branches together will produce an expected, non-logic `deploy.yml` conflict (each branch added its own job in the same file); resolve by keeping all job blocks, not by picking one side. Same applies to `plan.md` itself — each branch's Phase 5 status section only documents its own step.

**NEXT STEP when resuming:** (a) Step 8 — Microsite renderer (public, per-business subdomain, likely SSR/static-gen rather than SPA — see "Deployment architecture" above), or (b) merge `customer-app-scaffold`, `staff-pos-scaffold`, and `review-console-scaffold` into `main` (expect the `deploy.yml`/`plan.md` conflicts described above), or (c) a real npm install/build/run pass (optional, not blocking — user has confirmed CI/CD is trusted to catch issues). Business domain/subdomain names and Cloudflare account/project details are still not decided — confirm before wiring real `wrangler.toml` routes.
