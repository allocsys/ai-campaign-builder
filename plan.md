...
1. ~~**`ai_confidence_score`** on `task_submissions` is read by Review Console but nothing populates it in production yet.~~ — **CLOSED 2026-09-11.** Pipeline built (`lib/vision.ts` provider adapter, cascading Google's models then OpenAI's, `vision-cascade.config.ts`, PR #40) and wired into the submit route via `waitUntil`. Evidence storage built on **Backblaze B2**, redesigned to a **private bucket + server-side proxied downloads** (`storage.ts`'s `downloadEvidenceImage`, authenticated `GET /submissions/:id/evidence` proxy route on `review.ts`), merged via PR #41 + PR #42. **Confirmed live in production 2026-09-11:** verified directly against the `ai-campaign-builder-backend` Worker's bindings in the Cloudflare dashboard — all four `B2_*` secrets (`B2_KEY_ID`, `B2_APPLICATION_KEY`, `B2_BUCKET_ID`, `B2_BUCKET_NAME`) and `VISION_GOOGLE_API_KEYS` are set. `VISION_OPENAI_API_KEYS` is not set, which is fine by design — the cascade skips a provider with no keys configured rather than failing, so Google alone is enough for the pipeline to be fully functional; OpenAI remains an optional paid fallback for whenever Google's free-tier quota is exhausted. Every real submission now gets a real `ai_confidence_score` and real evidence upload/download end-to-end. No threshold bands for Item 5 yet — that's still open.
...
7. ~~**`apps/business-owner` has no public landing page**~~ — **CLOSED 2026-09-11.** Built a proper unauthenticated `LandingPage.tsx` at `/` (hero, feature grid, "how it works" steps, final CTA — all CTAs route to `/login`), with the authenticated app moved to `/dashboard`. `AuthScreen` is now the secondary "sign in" destination, not the front door: it redirects to `/dashboard` on success, and already-authenticated visitors hitting either `/` or `/login` bounce straight to `/dashboard` instead of seeing marketing copy or the OTP screen again. Merged via PR #43 (squash commit `b8ccba2`), branch `feature/business-owner-landing-page`, CI green.
...
16. **`review_admin` has account-administration access only (manage `review_team_members`/`review_admins`) but no campaign access at all** -- flagged 2026-09-12/13. Business owners are currently the only role that can view/edit a business's campaign. User wants `review_admin` to have **full parity** with a business owner's campaign access ("I'm the one making changes so it's full access" -- no partial/read-only subset), plus a redesigned `apps/review-console` admin UI to use it, modeled loosely on the business-owner app's campaign UI.
    ...
    **Built as a sequential chain, each step its own branch/PR, no step starting before the previous merges** (same convention as Items 13/14):
    ...
    - **Step D -- investigate the frontend model before building. DONE 2026-09-13, investigation only (no branch/PR -- read-only).** Confirmed current file locations: business-owner's campaign UI is `apps/business-owner/src/routes/tabs/DashboardTab.tsx` (mounted at `/dashboard`, read-only summary of the active campaign: hero, stats, tasks list, rewards list -- no per-field edit controls anywhere) plus `CampaignWizardTab.tsx` (mounted at `/dashboard/campaign`, the 5-step generate wizard from Item 8 -- calls `generateCampaign` then, on "launch", `updateCampaign(apiClient, { status: 'active' })` only). **Key finding: there is no manual field-level campaign editor anywhere in the business-owner app.** `updateCampaign`/`PUT /campaign` accepts an arbitrary `Partial<Campaign>` and the backend supports it, but the only caller in production passes `{ status: 'active' }` -- owners edit a campaign exclusively by re-running the wizard (regenerate) and launching, never by directly editing tasks/rewards/points/thresholds. So "full parity" for `review_admin` cannot mean 1:1-copy an existing rich editor, because one doesn't exist to copy -- Step E needs an explicit decision on what parity means here (see below). Also confirmed: `apps/review-console`'s admin surface is a single route (`/admin` -> `AdminHome.tsx`, gated by `AdminProtectedRoute`) with zero campaign-related UI today -- purely `review_team_members`/`review_admins` roster management via `admin-api-client.ts` (thin wrapper re-exporting `packages/api-client`'s review-admin resource functions, same pattern Step C's new functions will slot into). No existing business-picker component anywhere in the codebase (business-owner's JWT already scopes to one business, so it's never needed one) -- Step E will need to build one from scratch against Step C's `getAdminBusinesses()`.
      ...
      **Open question for Step E (needs a decision before building, not a guess):** given DashboardTab+CampaignWizardTab together *are* business-owner's full campaign "access" (view via Dashboard, change via regenerate-and-launch), does review_admin's "full campaign-access parity" mean: **(i)** mirror exactly that -- a read-only campaign view + a copy of the regenerate/launch wizard, scoped by a new business-picker, or **(ii)** go further and build the manual field-level editor (task points, reward thresholds, dates, status) that business-owner itself has never had, using the already-generic `PUT /campaign` endpoint's full `Partial<Campaign>` support. (i) is truer to "parity" (matches what owners can actually do today) and less new surface; (ii) is a strictly bigger scope than what's being mirrored and would give admins a capability owners don't have themselves. Flagging this now rather than assuming, same as Item 16's other decisions.
    - **Step E -- build the review-console admin campaign UI**, informed by Step D, using Step C's client methods. Not started -- blocked on the open question above.
