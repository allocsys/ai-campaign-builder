...
- **2026-09-10** — Step 10 (`apps/backend` scaffold + CI/CD deploy job) built on branch `backend-scaffold`. While getting the branch's CI green: found and fixed a stale root `package-lock.json` (never regenerated after `apps/backend/package.json` was added, breaking `npm ci` for every app's job, not just backend's) and three `tsc` errors in `src/middleware/auth.ts` (`Omit<>` losing property types over an indexed type, a bare `Uint8Array` not assignable to `BufferSource`, a missing `Variables` key on the Hono `Context` generic). Root cause noted: `.github/workflows/sync-lockfile.yml` only auto-syncs the lockfile on pushes to `main`, never on feature branches or as a PR check — a structural gap worth revisiting (not yet proposed to user). PR #24 merged to `main` (commit `d5f83d0`) after confirming all checks green on HEAD.
- **2026-09-10** — First real `main`-branch deploy after the Step 10 merge failed: `backend-migrate` errored because `wrangler d1 create <name> --json` isn't supported on the wrangler version pinned in `apps/backend/package.json` (only `d1 list --json` is) — wrangler printed usage text to stdout instead of JSON, breaking the `ensure-d1-database` action's parser. Fixed by dropping `--json` from the `create` call and reusing the already-working `d1 list --json` lookup to resolve the new database's id afterward; pushed directly to `main` (commit `8cd664a`, no PR — mirrors how `sync-lockfile.yml`'s own auto-commits land). User then added the also-missing `BACKEND_JWT_SECRET` repo secret. A manual `workflow_dispatch` run (`34426560345`) confirmed every job green end-to-end, including `backend-migrate` and `backend` for real (path-filtered push-triggered runs don't exercise jobs whose folder didn't change, so a manual dispatch was needed to actually test this). Backend is now live and deployed on Cloudflare.
...
**Step 10 done (2026-09-10), branch `backend-scaffold`, merged via PR #24 (commit `d5f83d0`):** `apps/backend` — the real API — scaffolded and wired into CI/CD for the first time (previously only a placeholder in the deploy.yml comments/apps folder listing). Hono app on Cloudflare Workers, D1-bound. Contents: `package.json`/`tsconfig.json`/`wrangler.toml` (D1 binding `DB`), `migrations/0001_init.sql` (all 35 architecture.md tables as SQLite DDL) + `migrations/0002_seed_config.sql` (business categories, task/reward patterns, website modules), `src/index.ts` (Hono app, CORS, mounts `/health` + `/api/auth`), `src/lib/db.ts` (D1 query helpers), `src/middleware/auth.ts` (signJWT/verifyJWT via Web Crypto HMAC-SHA256, `requireAuth` middleware), `src/routes/auth.ts` (dev-mode OTP stub reusing the existing frontend dev-OTP codes `7712`/`5432`/`9911`). `.github/workflows/deploy.yml` gained two new jobs: `backend-migrate` (runs `wrangler d1 migrations apply --remote`, gated on its own narrower `apps/backend/migrations/**` path filter) and `backend` (typecheck+test+deploy, waits on `backend-migrate` succeeding-or-skipped, fails fast if `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID`/`BACKEND_JWT_SECRET` repo secrets are missing, pushes `BACKEND_JWT_SECRET` to the deployed Worker via `wrangler secret put JWT_SECRET`). Also added `.github/actions/ensure-d1-database`, a composite action both new jobs call: looks the D1 database up by name via `wrangler d1 list`, creates it only if missing (idempotent — never recreates on redeploy), and patches that job's own local `wrangler.toml` checkout with the real `database_id` (never committed back — the repo's copy stays a placeholder forever; every job resolves the real id fresh).

Two real bugs surfaced and fixed while getting this branch's CI green, plus one more found only after merge (see Status Log for full detail): a stale root lockfile breaking `npm ci` repo-wide, three `tsc` errors in `auth.ts` (a TypeScript `Omit`-over-indexed-type quirk, a `Uint8Array`/`BufferSource` mismatch, a missing Hono `Variables` generic key), and — discovered on the very first real `main`-branch deploy post-merge — `wrangler d1 create` not supporting `--json` on the pinned wrangler version, breaking first-time D1 auto-provisioning. All three are fixed and confirmed resolved by a fully green manual `workflow_dispatch` run (run `34426560345`) exercising every job including `backend-migrate` and `backend` for real. The backend is now live and deployed on Cloudflare.

**Immediate follow-ups (2026-09-10, resolved same day):** `BACKEND_JWT_SECRET` repo secret — added by user. `wrangler.toml`'s D1 `database_id` placeholder — moot now that `ensure-d1-database` auto-provisions it per-job; no manual `wrangler d1 create` needed.

---

## Frontend-to-backend wiring (2026-09-10 onward)

Approach: build real backend endpoints + wire ONE full persona end-to-end at a time, business-owner first, then repeat the pattern for customer / staff-pos / review-console.

**`packages/api-client` scaffolded, PR #26 merged (commit `80e7b8d`):** shared `ApiClient` (`client.ts`, fetch wrapper with bearer-token injection + `ApiError`), `resources/auth.ts` (`requestOtp`/`verifyOtp`), `resources/business.ts`, `types.ts`. Used by every dashboard app going forward instead of hand-rolled fetch calls.

**Backend business routes + migrations 0003–0005, PR #27 merged to main:** `apps/backend/src/routes/business.ts` — profile, checklist, campaign (auto-provisioned draft on first read), insights, suggested-changes (apply/dismiss), autopilot, microsite (auto-provisioned), subscription (auto-provisioned), notifications-log. All scoped to `auth.sub` from the JWT so a business owner can only ever touch their own row. Verified directly against D1.

**business-owner persona fully wired, PR #28 merged (commit `9396447`):** `mock-data.ts` deleted for this app; all 8 tabs fetch through `packages/api-client`; `lib/auth.tsx` calls real `/api/auth/request-otp` + `/verify-otp` instead of the mock OTP stub; `.env.production` sets `VITE_API_BASE_URL` to the backend's workers.dev URL as a stopgap until a real domain is chosen; backend already has open CORS (`origin: "*"`) so this works cross-origin. **Not yet done:** an actual live browser smoke-test of business-owner against the deployed backend (CI typecheck/build is green, but no real network run-through yet) — still the one open gap on this persona.

### Customer persona — investigation done, implementation not started

User chose to start the **customer** persona next (before the business-owner smoke test). Investigated the existing repo state; here is the concrete plan for whoever/whatever resumes this:

**What already exists and needs no new work:**
- DB schema (migration 0001) already fully supports customers: `customers`, `customer_campaign_codes` (personal_code/qr_payload/referred_by_code_id), `task_submissions`, `purchase_logs`, `reward_redemptions`, `points_ledger` (source of truth for balance — no stored balance column anywhere, always `SUM(points)` over ledger rows for a code), `point_carryovers`, `referral_flags`, `notifications_log`. No new migration should be needed for the endpoints below.
- `apps/backend/src/routes/auth.ts` already handles `role: "customer"`: dev OTP `5432`, creates/looks up a `customers` row by phone on verify. Nothing to change here for basic login. (Referral-code-at-signup handling is NOT yet wired — see below.)
- `packages/api-client`'s `ApiClient`/`resources/auth.ts` work as-is for the customer app too.

**What's missing (all net-new):**
1. `apps/backend/src/routes/customer.ts` (new Hono sub-router, mounted at `/api/customer` in `src/index.ts`, gated by `requireAuth` + `role === "customer"`, same shape as `business.ts`). Needs, minimum, to match what `CustomerHome.tsx` currently reads from `mock-data.ts`:
   - An `ensureCustomerCampaignCode(db, customerId)` helper (mirrors `business.ts`'s `ensureCampaign`/`ensureMicrosite` auto-provision pattern) — since the customer app has no business/slug selector yet (single hardcoded demo business "کافه نارون", same as the mockup), this should resolve/join the *first* campaign in the DB, generating a `personal_code` + `qr_payload` the first time a given customer is seen. This is a deliberate simplification carried over from the current mock/single-tenant UI — flag it as a gap once a real multi-business customer join flow (via microsite slug) is needed.
   - `GET /api/customer/profile` → businessName (via join to `businesses`), personalCode, qrPayload, pointsBalance (`SUM(points_ledger.points)`), referralCount (`COUNT(customer_campaign_codes WHERE referred_by_code_id = mine)`), maxReferralCap (`campaigns.max_referrals_per_customer`), carryoverBonus (`SUM(point_carryovers.points)` unconsumed for this customer+business), telegramOptedIn.
   - `PUT /api/customer/telegram-opt-in` → sets `customers.telegram_opted_in = 1`.
   - `GET /api/customer/tasks` → `campaign_tasks` for the resolved campaign, joined to this code's own `task_submissions` for status (pending/approved/null).
   - `POST /api/customer/tasks/:id/submit` → creates a `task_submissions` row (`submission_type: 'screenshot'`, status `pending`). Matches `TaskSubmitModal.tsx`.
   - `POST /api/customer/tasks/:id/simulate-ai-approve` → dev-only stub mirroring the current UI's "شبیه‌سازی بررسی AI" button: immediately marks the submission `approved` + inserts a `points_ledger` `earned` row. Keep this clearly marked as a dev stub (same convention as `DEV_OTPS` in `auth.ts`) since real AI review isn't built yet.
   - `GET /api/customer/rewards` → `campaign_rewards` for the resolved campaign with progress computed against the profile's points balance.
   - `POST /api/customer/rewards/:id/redeem` → validates balance ≥ threshold, inserts `reward_redemptions` (5-minute `redemption_code_expires_at`) + a negative `points_ledger` `redemption` row, returns the code.
   - `POST /api/customer/retro-claims` → re-implements the three rules currently living client-side in `mock-data.ts`'s `submitRetroactivePurchaseClaim` (72h window, duplicate `receipt_hash` per code, max 3 pending claims) server-side against `task_submissions`(`submission_type: 'receipt_claim'`) + `purchase_logs`, since a client-only rate limit is meaningless once this is a real API.
   - `GET /api/customer/notifications` → `notifications_log` rows for this code, newest first, same shape as business-owner's `notifications-log` endpoint.
   - Referral-at-signup handling (`processReferralSignup` in current `mock-data.ts`) needs a real home: either folded into `verify-otp` (pass `referralCode` in the request body, look up the referrer's `customer_campaign_codes.personal_code`, set `referred_by_code_id`, enforce `max_referrals_per_customer` cap) or a separate `POST /api/customer/claim-referral` called once after first login. Decide which before implementing — folding into `verify-otp` matches the current frontend flow (`AuthScreen.tsx` passes `referralCode` straight into `verifyOtp`) most closely and avoids a second round-trip.
2. `packages/api-client/src/resources/customer.ts` + matching `types.ts` additions, one function per endpoint above, following `resources/business.ts`'s exact style (thin wrappers over `client.request`).
3. `apps/customer` frontend wiring, same shape as PR #28's business-owner change: delete `src/lib/mock-data.ts`'s stub state (keep only pure display types if still useful, or move them into `packages/api-client/src/types.ts`), rewrite `lib/auth.tsx` to call real `requestOtp`/`verifyOtp` with `role: 'customer'` (copy business-owner's `auth.tsx` pattern almost verbatim — the only real difference is passing `referralCode` through to `verifyOtp`, whatever shape step 1's decision above lands on), rewrite `CustomerHome.tsx`/`TaskSubmitModal.tsx`/`RetroClaimModal.tsx` to fetch/mutate via the new `resources/customer.ts` calls instead of local state, add `apps/customer/src/lib/api-client.ts` (copy business-owner's verbatim, swap the localStorage key to `aicb_customer_auth`), add `apps/customer/.env.production` (same `VITE_API_BASE_URL` stopgap value as business-owner) and `apps/customer/src/vite-env.d.ts` if missing (CI caught this exact gap once already for business-owner — don't skip it here).
4. Once wired: delete the dev-bypass button/block in `apps/customer/src/lib/auth.tsx` + `AuthScreen.tsx` per the standing open item, and keep the retro-claim rejection UX (`outside_time_window`/`duplicate_receipt`/`rate_limited`) identical — those three reasons should come back as the `POST /api/customer/retro-claims` response shape, not be re-derived client-side.

No backend/api-client/frontend code has been written yet for this persona — only the plan above. Next session should start with step 1 (`apps/backend/src/routes/customer.ts`), get it merged and verified against D1 directly (same verification style as PR #27), *then* move to steps 2–3, following the delegate_editor batching lessons in the checkpoint (small precise batches, exact signatures pre-looked-up, verify claims independently via `search_code` rather than trusting the agent's own summary).
