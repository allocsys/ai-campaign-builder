...
**Admin scope + UI — decided 2026-09-11, supersedes the earlier "v1 assumes a single main admin" note:** a **dedicated new UI on a new route**, separate from the existing review-console app, giving the admin **full access** to (a) register/deactivate `review_team_members` rows (the reviewer roster) and (b) **add/remove other admins** — meaning multiple admins are now in scope, not just the one root env-configured admin. This implies a persisted admins table beyond the single env-configured identity (working name: `review_admins` — id, username, password_hash, created_at, created_by), where the env-configured root admin is the bootstrap identity and can create further rows in that table through the new UI. The one confirmed user of this UI for now is the person making these decisions. Not yet decided: whether "a new route" means a new page inside the existing `apps/review-console` app (gated by the `review_admin` role) or an entirely separate app/Worker in the monorepo (matching the pattern of `apps/business-owner`, `apps/customer`, etc. each being their own app) — and, once `review_admins` exists as a real table, whether additional admins still log in with env-configured credentials or get their own username+password issued at creation time.
...
6. **`review_team` OTP verification has no roster/invite check** — unlike `staff`, any phone number can authenticate as `role: 'review_team'` (see Phase 0.5 "Security finding"). **Decided 2026-09-11: fix via `review_team_members`, built invite-only** (mirroring `staff`'s invite gate exactly). Registration is gated by a **separate `review_admin` role**, logging in with **username+password** (not phone+OTP) via its own new login route — the root admin's credentials are env-configured, solving the bootstrap problem (the first admin can't be invited by anyone). This admin gets a **dedicated new UI on a new route** with full access to manage `review_team_members` *and* to add/remove other admins (a `review_admins` table beyond the one env-configured root identity — multiple admins are in scope, not just one). Not yet implemented — needs the `review_team_members` + `review_admins` migrations, FK columns (Item 2), the `verify-otp` gate for `review_team`, the new admin login route + UI, and the admin's registration endpoint for reviewer/admin management.
