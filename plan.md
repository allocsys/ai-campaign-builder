...
6. **`review_team` OTP verification has no roster/invite check** — unlike `staff`, any phone number can authenticate as `role: 'review_team'` (see Phase 0.5 "Security finding"). **Decided 2026-09-11: fix via `review_team_members`, built invite-only** (mirroring `staff`'s invite gate exactly). Registration is gated by a **separate `review_admin` role with its own login route** (not a flag on `review_team_members` itself), whose one main admin identity is **env-configured** (e.g. `ADMIN_PHONE`) rather than a seeded roster row — solves the bootstrap problem, since the first admin can't be invited by anyone. Not yet implemented — needs the `review_team_members` migration + FK columns (Item 2), the `verify-otp` gate for `review_team`, the new `review_admin` login route, and the admin's actual registration endpoint/UI (review-console equivalent of `POST /api/business/staff`).
