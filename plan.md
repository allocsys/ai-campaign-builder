...
10.5. **Wizard's Step 3 size-tier signal renamed from "offer budget" to "monthly revenue"** -- decided 2026-09-12 (see Phase 1 "Business size-tier scaling" for the full rationale and revised threshold table). Not yet built: `resolveSizeTier`, `campaign-generator.ts`'s `GenerateCampaignInput`, `routes/business.ts`'s `/campaign/generate` body, `packages/api-client`'s request type, and the wizard's Step 3 field/label all still use the old `offerBudgetToman` name and the old (much smaller) threshold scale.
