...
8. **`apps/business-owner` has no onboarding wizard / AI campaign-generation flow** — flagged 2026-09-11. Phase 0 specs the full flow (4 core questions + 1 conditional question → weighted AI campaign proposal → Launch/Edit) and it was fully built in the click-through mockup (`mockup/business-owner.html`'s "ONBOARDING WIZARD" section + `shared/app.js`'s `resolveSizeTier`/`generateCampaignTasksForCategory`/`getSuggestedDuration` client-side logic), but **none of it exists in production**. `OnboardingTab.tsx` in the real app is a different, smaller feature (the post-launch "Next Steps" checklist from the "Post-launch guidance" decision, not the wizard). `DashboardTab.tsx` only displays a campaign; `apps/backend/src/routes/business.ts`'s `GET/PUT /campaign` auto-provisions an empty draft campaign on first read and lets a caller overwrite tasks/rewards directly, but there is no UI entry point to create one, and no generation logic at all server-side: no `resolveSizeTier`-equivalent, no `task_pattern`/category weighting-table data seeded in D1 (Phase 1's 0-3 weighting table exists only as a markdown table in this doc and as hardcoded JS in the mockup), no endpoint that takes onboarding answers and returns a weighted proposal. A business owner today has no way to answer the onboarding questions or launch a campaign through the real product — the dashboard just shows an empty campaign shell. **Not yet built.**
...
**Still not started:** no migration written yet for seeding `category_pattern_weights`, no generation endpoint, no wizard UI. This entry records the design decisions only — implementation is a separate, explicit next step.
