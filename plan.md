...
7. **`apps/business-owner` has no public landing page** — flagged 2026-09-11. `App.tsx`'s `/` route is wrapped in `ProtectedRoute`, so an unauthenticated visitor is redirected straight to `/login` (a bare phone+OTP screen, no branding/value-prop/CTA). There is no self-signup story today — a new business owner has to already know to expect the OTP screen, presumably reached via a manually-shared URL. **NEXT UP:** build a proper unauthenticated landing page at `/` (what the platform does, "Get Started" CTA into the existing OTP flow), with the current `AuthScreen` becoming a secondary "sign in" destination rather than the front door. Not yet started — no design/copy decisions made yet.
