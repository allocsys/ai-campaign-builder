...
- [x] **Decided (2026-09-07), superseded during build (2026-09-11):** originally planned as a separate microservice with a job queue between it and the main API, so review load/latency and future AI-provider changes would stay isolated from the main API. **What was actually built (plan.md Open Item 1, PR #39–41):** inline in the main API, not a separate service. `routes/customer.ts`'s `POST /tasks/:id/submit` fires the vision call via Cloudflare Workers' `c.executionCtx.waitUntil(...)` after the submission row is inserted, so it doesn't block the customer's response but also doesn't need a separate deployable, queue, or cross-service auth. `lib/vision.ts`'s provider-cascade design (Google→OpenAI, comma-separated key lists) covers the "isolate AI-provider changes" goal the microservice split was meant to solve, without the operational overhead of a second service. Revisit an actual microservice split only if `waitUntil` latency/cost becomes a real constraint.
