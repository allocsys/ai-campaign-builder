...
| address | text, nullable | free-text business address (plan.md Open Item 9, PR #47) — collected via the onboarding wizard's Step 1 or the Settings profile form, whichever the owner reaches first; auto-fills the microsite Contact module's address field and is exposed on the business profile response. Also documented as an available `{{business_address}}` placeholder for `notification_templates.body_template`, though no code renders `body_template` into a sent message yet (see `notifications_log` note below) |
...
- [ ] **Decided (2026-09-07), NOT achieved as built (flagged 2026-09-12) — load isolation still outstanding.** Original plan: separate microservice with a job queue between it and the main API, specifically so a burst of review load couldn't compete with/degrade the main API's other traffic. **What was actually built (plan.md Open Item 1, PR #39–41):** inline in the main API, not a separate service — `routes/customer.ts`'s `POST /tasks/:id/submit` fires the vision call via Cloudflare Workers' `c.executionCtx.waitUntil(...)` after the submission row is inserted. This solves *not blocking the customer's response* and `lib/vision.ts`'s provider-cascade design (Google→OpenAI, comma-separated key lists) solves *isolating AI-provider changes* — but neither solves the original **load-balancing** goal: `waitUntil` work still runs inside the same Worker and competes for the same CPU/invocation budget as every other backend route under a submission surge. This was written up in plan.md Item 1 as a clean implementation note at the time, without flagging that the load-isolation goal of the original architecture decision was quietly dropped, not achieved. **User decision 2026-09-12: defer, not urgent — actually split review-scoring into a separate Worker (with a real queue, e.g. Cloudflare Queues) at a later date, once load actually justifies it.** Until then, be aware a submission surge can degrade other backend routes' latency.
